...
const unsigned int CCValues[5] = {0x31, 0x32, 0x33, 0x0A, 0x34}; //BIT, FLD, DRV, PAN, GAN //Complete Midi implementation on https://www.korg.com/us/support/download/product/0/809/
...
void setup(){
	...
	for (int j = 0; j < numReadings; j++) {
		digitalWrite(ledPort[j%6], HIGH);
		for (int i = 0; i < 6; i++){
			readings[i][j] = analogRead(potPort[i]);
			total[i] += readings[i][j];
		}
		delay(50); 
		digitalWrite(ledPort[j%6], LOW);
		delay(50);
	} 
